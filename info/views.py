...
from .models import Department, Class, Student, Attendance, Course, Teacher, Assign, AttendanceTotal, time_slots, DAYS_OF_WEEK, AssignTime, AttendanceClass, StudentCourse, Marks, MarksClass
...
@login_required
def semeter1(request):
    return render(request,'info/semeter1.html')

@login_required()
def attendance(request, stud_id):
    stud = Student.objects.get(EID=stud_id)
...
def attendance_detail(request, stud_id, course_id):
    stud = get_object_or_404(Student, EID=stud_id)
    ...
    for i, s in enumerate(cl.student_set.all()):
        ...
        if assc.status == 1:
            try:
                ...
                a.save()
                try:
                    a= AttendanceTotal.objects.get(student=s,course=cr)
                    if a.attendance < 85 and a.total_class>=5:
                        send_mail('ATTENDANCE LOW!!!', 'Your attendance is low in ' + a.course_id + ' ' + a.course.name
                                  + '. Please visit your student portal to know the exact details.',
                                  'iiitlcollege@gmail.com', [s.email], fail_silently=False)
                except AttendanceTotal.DoesNotExist:
                    a = AttendanceTotal(student = s,course = cr)
...
def change_att(request, att_id):
    ...
    a.save()
    return HttpResponseRedirect(reverse('t_attendance_detail', args=(a.student.EID, a.course_id)))
...
def marks_list(request, stud_id):
    ...
    for ass in ass_list:
        try:
            ...
            sc.marks_set.create(type='E', name='Semester Mid Exam')
...
def sem1(request,class_id):
    asst = AssignTime.objects.filter(assign__class_id=class_id)
    return render(request, 'info/sem1.html')


def sem2(request,class_id):
    asst = AssignTime.objects.filter(assign__class_id=class_id)
    return render(request, 'info/sem2.html')


def sem3(request,class_id):
    asst = AssignTime.objects.filter(assign__class_id=class_id)
    return render(request, 'info/sem3.html')


def sem4(request,class_id):
    asst = AssignTime.objects.filter(assign__class_id=class_id)
    return render(request, 'info/sem4.html')


def sem5(request,class_id):
    asst = AssignTime.objects.filter(assign__class_id=class_id)
    return render(request, 'info/sem5.html')
